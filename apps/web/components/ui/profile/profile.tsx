...
// Mock data for "Avis laissés" (reviews left by user)
const mockAvisLaisses: Review[] = [
  {
    id: 'mock-1',
    content: "Super trajet avec ce chauffeur ! Très ponctuel et voiture très propre. Je recommande vivement.",
    rating: 5,
  ...
  {
    id: 'mock-2',
    content: "Bonne expérience globale. Le chauffeur était sympathique et la conduite agréable.",
    rating: 4,
  ...
  {
    id: 'mock-3',
    content: "Trajet correct mais un peu de retard au départ. Sinon RAS.",
    rating: 3,
...
// Mock data for "Avis reçus" when no real reviews exist
const mockAvisRecus: Review[] = [
  {
    id: 'mock-received-1',
...
export default function ProfileComponent({ variant }: ProfileProps) {
  ...
  // Get reviews - use real data if available, otherwise use mock data
  const realReviews = (user?.review || []) as Review[];
  const avisRecus = realReviews.length > 0 ? realReviews : mockAvisRecus;
  const avisLaisses = mockAvisLaisses; // Always use mock for now until backend supports this
  ...
  // User profile - simple gallery without sub-tabs
  ...
  // Driver profile - gallery with sub-tabs (Chauffeur/Véhicules)
  const renderDriverPhotoSubContent = () => {
  ...
  const renderDriverPhotos = () => {
    return (
      <>
        ...
        <div className={styles.gallerySection}>{renderDriverPhotoSubContent()}</div>
      </>
    );
  };

  const renderReviewCard = (review: Review, showEditIcon: boolean = false) => (
    <div key={review.id} className={styles.reviewCard}>
      <div className={styles.reviewHeader}>
        ...
        {showEditIcon && (
          <button className={styles.editReviewButton} aria-label="Modifier">
            ✎
          </button>
        )}
      </div>
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'photos':
        return isDriver ? renderDriverPhotos() : renderUserPhotos();
      case 'avis_recus':
        return (
            ...
            {avisRecus.length > 0 ? (
              <>
                <div className={styles.reviewsGrid}>
                  {avisRecus.map((review: Review) => renderReviewCard(review))}
                </div>
        ...
        return (
          <div className={styles.reviewsSection}>
            {avisLaisses.length > 0 ? (
              <>
                <div className={styles.reviewsGrid}>
                  {avisLaisses.map((review: Review) => renderReviewCard(review, true))}
                </div>
                <div className={styles.loadMoreContainer}>
                  <button className={styles.loadMoreButton}>Charger plus</button>
                </div>

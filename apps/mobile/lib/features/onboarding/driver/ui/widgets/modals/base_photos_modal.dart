...
abstract class BasePhotosModal extends StatefulWidget {
  final List<File> selectedImages;
  final Function(List<File>) onImagesChanged;

  const BasePhotosModal({
    super.key,
...
abstract class BasePhotosModalState<T extends BasePhotosModal>
    extends State<T> {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: _localImages.isEmpty ? buildEmptyState() : buildGallery(),
          ),
          const SizedBox(height: 16),
          if (_localImages.length > 1) buildPageIndicator(),
          const SizedBox(height: 16),
          buildActionButtons(),
        ],
      ),
    ...
    return Stack(
      children: [
        ClipRRect(
          ...
          child: kIsWeb ? buildWebGallery() : buildMobileGallery(),
        ),
        Positioned(
          top: 16,
          right: 16,
  ...
  Widget buildImageError(int index) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Icon(
  ...
  // Getters pour accéder aux données depuis les classes filles
  List<File> get localImages => _localImages;
  int get currentIndex => _currentIndex;

  // Méthode pour ajouter une image (utilisée par PhotoManagementModal)

...
class StorageService {
  ...
  Future<void> storePhotos(List<dynamic> photos, String type) async {
    try {
      ...
      final typeDirectory = Directory(
        path.join(directory.path, 'driver_documents', type),
      );
      ...
      for (int i = 0; i < photos.length; i++) {
        final file = photos[i];
        final fileName =
            '${type}_${DateTime.now().millisecondsSinceEpoch}_$i${path.extension(file.path)}';
  ...
  Future<void> clearAllPhotos() async {
    try {
      ...
      final documentsDirectory = Directory(
        path.join(directory.path, 'driver_documents'),
      );

...
class OrsRouteProvider implements IRouteProvider {
  ...
  OrsRouteProvider({OrsClient? client})
    : _client =
          client ??
          OrsClient(
            baseUrl: MapConfig.orsBaseUrl,
            apiKey: MapConfig.orsApiKey,
            countryBoundary: MapConfig.orsCountryBoundary,
          );

  @override
  Future<Map<String, dynamic>?> forwardGeocodeRaw(String query) =>
      _client.forwardGeocode(query);

  @override
  Future<Map<String, dynamic>?> reverseGeocodeRaw(double lat, double lon) =>
      _client.reverseGeocode(lat, lon);

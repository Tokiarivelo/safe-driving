...
import 'package:safe_driving/features/home/map/core/interfaces/i_driver_service.dart'
    as map_driver;
...
void registerMapModule(ServiceLocator sl) {
  ...
  sl.registerLazySingleton<IMapDataSource>(
    () => MapDataSource(routeProvider: sl.get<IRouteProvider>()),
  );
  sl.registerLazySingleton<IMapService>(
    () => MapService(dataSource: sl.get<IMapDataSource>()),
  );

  sl.registerLazySingleton<IDeviceLocationService>(
    () => DeviceLocationServiceGeolocator(),
  );
  sl.registerLazySingleton<FiltersLocalDataSource>(
    () => FiltersLocalDataSource(),
  );
  sl.registerLazySingleton<FiltersService>(
    () => FiltersService(sl.get<FiltersLocalDataSource>()),
  );
  sl.registerLazySingleton<PositionLocalDataSource>(
    () => PositionLocalDataSource(),
  );
  sl.registerLazySingleton<PositionPersistenceService>(
    () => PositionPersistenceService(sl.get<PositionLocalDataSource>()),
  );

  if (GraphQLConfig.isConfigured) {
    sl.registerLazySingleton<IPositionDataSource>(
      () => PositionDataSourceGraphQL(sl.get<GraphQLClientWrapper>()),
    );
    sl.registerLazySingleton<IPositionService>(
      () => PositionService(sl.get<IPositionDataSource>()),
    );
    sl.registerLazySingleton<map_driver.IDriverService>(
      () => MapServiceSocketIO(),
    );
    ...
    sl.registerLazySingleton<MapRepository>(
      () => MapRepository(service: sl.get<IMapService>()),
    );
  }
